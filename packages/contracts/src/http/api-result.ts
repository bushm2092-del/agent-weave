...
export type ApiResult<T, C extends string = string> =
  { ok: true; data: T; meta: ApiMeta } | { ok: false; error: ApiError<C>; meta: ApiMeta }
...
export function apiFailure<C extends string>(error: ApiError<C>, requestId: string): ApiResult<never, C> {
