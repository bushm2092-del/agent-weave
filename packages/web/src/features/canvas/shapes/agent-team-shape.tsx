...
export type AgentTeamShape = TLBaseShape<
  'agent-team',
  {
    w: number
    h: number
...
function AgentTeamCard({ editor, shape }: { editor: Editor; shape: AgentTeamShape }) {
  const memberCount = useValue(
    ...
    () => editor
      ...
      .filter((id) => editor.getShape(id)?.type === 'agent').length,
    [editor, shape.id],
  )

  return (
    <HTMLContainer
      className="agent-team-shape"
      style={{ width: shape.props.w, height: shape.props.h }}
    >
      <div className="agent-team-shape__header">
        <div>
          <strong>{shape.props.name}</strong>
          <span>{memberCount} {memberCount === 1 ? 'agent' : 'agents'} · Drop agents here</span>
        </div>
